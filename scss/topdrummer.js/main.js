...
function addToCart(obj){
    var result = confirmado for (let index = 0; index < array.length; index++) {
...
while (condition);(Añadir producto al carrito de la compra? ');
    if (result == false)){
        return;
    }
 ...
 //Sustracción
    function minus(index) {
        ...
        var amounts=document.getElementsByName("amount");
                 ...
                 var count = parseInt (montos [índice] .valor); // Monto más 1
 
        if (count<=1){
                         alert ("¡No se puede reducir más, casi se ha ido!");
        } else {
                         // Obtiene el valor del atributo value del elemento de la primera cantidad
                         var count = parseInt (montos [índice] .valor) -1; // Monto más 1
